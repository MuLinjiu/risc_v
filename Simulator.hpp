...
public:
    ...
    void ID(){
        ...
        if(BT == R){
            ...
            unsigned int rs1 = (preID.command >> 15) & 31;
            unsigned int rs2 = (preID.command >> 20) & 31;
            unsigned int imm = ((preID.command >> 31) << 12) + (((preID.command >> 7) & 1) << 11) + (((preID.command >> 25) & 63) << 5) + (((preID.command >> 8) & 15) << 1);
            imm = sext(13,imm);
            preEX.rs1 = rs1;
            preEX.rs2 = rs2;
            preEX.imm = imm;
            ...
            preEX.rd = (preID.command >> 7) & 31;
            unsigned int imm = ((preID.command >> 31) << 20 ) + (((preID.command >> 12) & 255) << 12) + (((preID.command >> 20) & 1) << 11) + (((preID.command >> 21) & 1023) << 1);
            imm = sext(21,imm);
            preEX.imm = imm;
        }

//        if(preEX.rs1 == preWB.rd){
//            preEX.rs1_value = preWB.value;
//        }
//        if(preEX.rs2 == preWB.rd){
//            preEX.rs2_value = preWB.value;
//        }
//        if(preEX.rs1 == preMEM.rd){
//            preEX.rs1_value = preMEM.value;
//        }
//        if(preEX.rs2 == preMEM.rd){
//            preEX.rs2_value = preMEM.value;
//        }
    }
    void EX(){
